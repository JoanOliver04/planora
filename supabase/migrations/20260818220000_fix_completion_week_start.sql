-- Qualify profiles.week_starts_on so it is not confused with the
-- PL/pgSQL variable of the same name (ambiguous column reference).
...
begin
  ...
  select timezone, coalesce(public.profiles.week_starts_on, 1)
    into user_timezone, profile_week_start
  ...
  then
    ...
    week_start = new.occurrence_date
      - ((extract(dow from new.occurrence_date)::integer
        - coalesce(profile_week_start, 1) + 7) % 7);
