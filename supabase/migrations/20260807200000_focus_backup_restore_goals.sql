-- Focus restore v4: flexible goals + preset management columns on atomic restore.
-- Live sessions remain terminalized by the app layer before this RPC runs.
...
begin
  ...
  -- Release deferred profile FK first, then dependants in FK-safe order.
  ...
  insert into public.focus_presets(
    id, user_id, name, emoji, intention, mode, focus_duration_sec,
    short_break_sec, long_break_sec, cycles_before_long_break, target_cycles,
    auto_start_breaks, auto_start_focus, sound_enabled, vibration_enabled,
    notify_on_phase_end, complete_task_on_session_end, keep_screen_awake,
    prefer_fullscreen, segments, is_favorite, sort_order, default_category_id,
    archived_at
  )
  select x.id, current_user_id, x.name, x.emoji, x.intention, x.mode,
    x.focus_duration_sec, x.short_break_sec, x.long_break_sec,
    x.cycles_before_long_break, x.target_cycles, x.auto_start_breaks,
    x.auto_start_focus, x.sound_enabled, x.vibration_enabled,
    x.notify_on_phase_end, x.complete_task_on_session_end, x.keep_screen_awake,
    x.prefer_fullscreen, coalesce(x.segments, '[]'::jsonb), x.is_favorite,
    x.sort_order, x.default_category_id, x.archived_at
  from jsonb_to_recordset(focus_presets_data) as x(
    id uuid, name text, emoji text, intention text, mode text,
    focus_duration_sec integer, short_break_sec integer, long_break_sec integer,
    ...
    default_category_id uuid, archived_at timestamptz
  );
