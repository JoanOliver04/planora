-- Extend atomic restore with Focus entities (presets, sessions, intervals, goals)
-- and persist task scope. Live sessions are expected to be terminalized by the
-- app layer before this RPC runs (cancelled + closed intervals).
...
begin
  ...
  insert into public.focus_presets(
    ...
    segments, is_favorite, sort_order
  )
  ...
  insert into public.focus_goals(
    id, user_id, period, target_focus_sec, timezone, week_starts_on, active
  )
  select x.id, current_user_id, coalesce(nullif(x.period, ''), 'weekly'),
    x.target_focus_sec, x.timezone, x.week_starts_on, x.active
  from jsonb_to_recordset(focus_goals_data) as x(
    id uuid, period text, target_focus_sec integer, timezone text,
    week_starts_on smallint, active boolean
  );
