...
Planora exports UTF-8 JSON backups with explicit metadata (`format`, `schemaVersion`, `backupId`, `createdAt`, `exportedBy`, locale and timezone), separate CSV tables, and an RFC 5545-compatible ICS calendar. Version 5 is the current JSON contract: Focus entities (`focus_presets`, `focus_sessions`, `focus_intervals`, flexible `focus_goals`), guided-plan modes, nullable task preset recommendations, orphan Focus FK sanitization, and privacy-oriented CSV (notes in a separate PRIVATE file). Versions 2–4 upgrade in memory; v2 receives empty Focus collections and v4 recommendations default to null. Each task record includes `scope` (`schedule` or `global`). Global tasks use `schedule_id: null`; schedule tasks require a valid `schedule_id`. Backups without `scope` are treated as schedule-scoped for backward compatibility. Genuine version 1 exports are migrated in memory, validated against the current model, and never modified on disk.
