...
export const taskSchema = z
  .object({
    ...
    timing: z.discriminatedUnion("mode", [
      ...
      z.object({
        mode: z.literal("day_part"),
        dayPart: z.enum(["morning", "afternoon", "night"]),
      }),
      z.object({
        mode: z.literal("specific_time"),
  ...
  .superRefine((v, c) => {
    ...
    if (
      v.timing.mode === "time_range" &&
      v.timing.startTime >= v.timing.endTime
    )
      c.addIssue({
        code: "custom",
        message: "Time ranges cannot cross midnight",
        path: ["timing", "endTime"],
      });
  });
